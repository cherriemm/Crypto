...
void sm4_cipher(uint8_t *in, uint8_t *out) {
    uint8_t x[36][4] = {};
    for (int i = 0; i < 16; ++i) {
        x[i / 4][i % 4] = in[i]; /*补充SM4加密函数*/;
    }
    for (int i = 0; i < ROUND; ++i) {
        uint8_t temp[4] = {};
        uint32_t res = 0;
        for (int j = 0; j < 4; ++j) {
            uint8_t index = x[i + 1][j] ^ x[i + 2][j] ^ x[i + 3][j] ^ rk[i][j];/*补充SM4加密函数*/
            temp[j] = sbox[index / 16][index % 16];/*补充SM4加密函数*/;
        }
        GET_ULONG_BE(res, temp, 0)
        res = res ^ ((res << 2) | (res >> 30)) ^ ((res << 10) | (res >> 22)) ^ ((res << 18) | (res >> 14)) ^ ((res << 24) | (res >> 8));/*补充SM4加密函数*/;
        PUT_ULONG_BE(res, temp, 0)
        for (int l = 0; l < 4; ++l) {
            x[i + 4][l] = x[i][l] ^ temp[l];/*补充SM4加密函数*/;
        }
    }
    for (int i = 0; i < 4; ++i) {
    	for (int j = 0; j < 4; ++j)
    	{
    		out[i*4 + j] = x[35 - i][j];/*补充SM4加密函数*/;
    	}
    }
}
