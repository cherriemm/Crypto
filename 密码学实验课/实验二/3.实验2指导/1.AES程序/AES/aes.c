...
// 补全AES加密代码
  ...
  for (r = 1; r < Nr; r++) {
//  补全AES加密1-9轮代码
    sub_bytes(state);            // 1. SubBytes: Byte substitution using S-box
    shift_rows(state);           // 2. ShiftRows: Row-wise circular shifts
    mix_columns(state);          // 3. MixColumns: Column-wise linear transformation
    add_round_key(state, w, r);   // 4. AddRoundKey: Add round key (XOR with expanded key)
  }
    
// 补全AES加密第10轮代码
   sub_bytes(state);                // 1. SubBytes
   shift_rows(state);               // 2. ShiftRows
   add_round_key(state, w, Nr);      // 3. AddRoundKey for the final round
